...
- [ ] Выполнить `./scripts/project-menu.sh --help` и проверить menu tests.
...
- [ ] После BIOS cleanup проверить путь `no enrolled fingerprints` -> `./scripts/enroll-test.sh --finger right-index-finger "$USER"` -> `fprintd-verify "$USER"`.
- [ ] Проверить финальную ручную матрицу: `sudo -k && sudo true`, неверный палец, password fallback, GNOME lock/unlock.
- [ ] Не считать GDM fingerprint login production-ready на autologin setup; если проверялся и завис, зафиксировать это как known risk, не как release blocker для sudo/lock scope.
