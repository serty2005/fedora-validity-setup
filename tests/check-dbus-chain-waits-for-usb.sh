...
if [[ "$(cat "$TMPDIR/lsusb-count")" != "3" ]]; then
  printf 'expected lsusb to be retried until the USB device appears\n' >&2
  exit 1
fi

grep -F '[ok] Bus 001 Device 012: ID 138a:0097 Validity Sensors, Inc.' <<<"$OUTPUT" >/dev/null
